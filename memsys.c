...
uns64 memsys_access_modeBC(Memsys *sys, Addr lineaddr, Access_Type type){
  ...
  if(type == ACCESS_TYPE_IFETCH){
    ...
    if (outcome == MISS) {
      ...
      delay += memsys_L2_access(sys, lineaddr, 0);
    }
  }
  ...
  if(type == ACCESS_TYPE_LOAD){
    // YOU NEED TO WRITE THIS PART AND UPDATE DELAY
    Flag outcome = cache_access(sys->dcache, lineaddr, 0);
    delay += DCACHE_HIT_LATENCY;
    if (outcome == MISS) {
      delay += memsys_L2_access(sys, lineaddr, 0);
      cache_install(sys->dcache, lineaddr, 0);
      if (sys->dcache->last_evicted_line.valid && sys->dcache->last_evicted_line.dirty) {
        memsys_L2_access(sys, sys->dcache->last_evicted_line.tag * sys->dcache->num_sets + lineaddr % sys->dcache->num_sets, 1);
      }
    }
  }
  ...
  if(type == ACCESS_TYPE_STORE){
    // YOU NEED TO WRITE THIS PART AND UPDATE DELAY
    Flag outcome = cache_access(sys->dcache, lineaddr, 1);
    delay += DCACHE_HIT_LATENCY;
    if (outcome == MISS) {
      delay += memsys_L2_access(sys, lineaddr, 0);
      cache_install(sys->dcache, lineaddr, 1);
      if (sys->dcache->last_evicted_line.valid && sys->dcache->last_evicted_line.dirty) {
        memsys_L2_access(sys, sys->dcache->last_evicted_line.tag * sys->dcache->num_sets + lineaddr % sys->dcache->num_sets, 1);
      }
    }
...
uns64   memsys_L2_access(Memsys *sys, Addr lineaddr, Flag is_writeback){
  ...
  Flag outcome = cache_access(sys->l2cache, lineaddr, is_writeback);
  if (outcome == MISS) {
    ...
    if (sys->l2cache->last_evicted_line.valid && sys->l2cache->last_evicted_line.dirty) {
      dram_access(sys->dram, sys->l2cache->last_evicted_line.tag * sys->l2cache->num_sets + lineaddr % sys->l2cache->num_sets, 1);
    }
  }
